...
The account owner approved launch without a retry schedule or a real booking check. Retries have no scheduled worker, so queued or uncertain notifications may need manual attention; saved inquiries remain in Supabase. Real booking behavior remains unverified. Pending: replacement of the four Production/general Preview public Config entries and production deployment verification. Saved environment variables require a new deployment before they take effect.
