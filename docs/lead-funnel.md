...
The four `NEXT_PUBLIC_EMAILJS_*` values are saved as **Config** for Production and general Preview, with the working contractor-branch overrides retained. With the account owner's approval, the old Secret entries were replaced using the verified public values from `docs/emailjs-setup.md`. All four names, types, and scopes were verified in Vercel after saving. The private EmailJS/Supabase keys and hashing/retry secrets remain Secret.
...
The account owner approved launch without a retry schedule or a real booking check. Retries have no scheduled worker, so queued or uncertain notifications may need manual attention; saved inquiries remain in Supabase. Real booking behavior remains unverified. The production configuration is ready. Merge and deployment status is tracked in PR #9. Saved environment variables require a new deployment before they take effect.
...
5. Optional: configure scheduled email retries. The account owner chose to defer this for the initial launch. In Supabase, enable Cron and pg_net. In Vault, add `kredance_lead_retry_url` containing the final HTTPS `/api/leads/retry` URL and `kredance_lead_retry_token` containing the matching `CRON_SECRET`. Review and run `supabase/enable-lead-retries.sql`. It schedules a request every five minutes, avoiding a dependency on Vercel's Cron plan. Monitor HTTP results, not only cron execution success. Preview deployments protected by Vercel authentication need an appropriate authenticated test scheduler; do not disable production access protections for a test.
